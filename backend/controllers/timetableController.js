import asyncHandler from 'express-async-handler';
import Timetable from '../models/Timetable.js';
import { getIO } from '../services/socketService.js';

// @desc    Create a timetable entry
// @route   POST /api/timetables
// @access  Private/Admin
const createTimetableEntry = asyncHandler(async (req, res) => {
    const { courseCode, courseTitle, day, startTime, endTime, lecturer, venue, level, department } = req.body;

    const entry = new Timetable({
        courseCode,
        courseTitle,
        day,
        startTime,
        endTime,
        lecturer,
        venue,
        level,
        department,
        createdBy: req.user._id, // This line automatically sets the creator
    });

    const createdEntry = await entry.save();
    getIO().emit('new_timetable_entry', createdEntry);
    res.status(201).json(createdEntry);
});

// @desc    Get all timetable entries
// @route   GET /api/timetables
// @access  Public
const getTimetableEntries = asyncHandler(async (req, res) => {
    const entries = await Timetable.find({}).populate('createdBy', 'name').sort({ startTime: 1 });
    res.json(entries);
});

// @desc    Update a timetable entry
// @route   PUT /api/timetables/:id
// @access  Private/Admin
const updateTimetableEntry = asyncHandler(async (req, res) => {
    const { courseCode, courseTitle, day, startTime, endTime, lecturer, venue, level, department } = req.body;

    const entry = await Timetable.findById(req.params.id);

    if (entry) {
        entry.courseCode = courseCode || entry.courseCode;
        entry.courseTitle = courseTitle || entry.courseTitle;
        entry.day = day || entry.day;
        entry.startTime = startTime || entry.startTime;
        entry.endTime = endTime || entry.endTime;
        entry.lecturer = lecturer || entry.lecturer;
        entry.venue = venue || entry.venue;
        entry.level = level || entry.level;
        entry.department = department || entry.department;

        const updatedEntry = await entry.save();
        getIO().emit('timetable_entry_updated', updatedEntry);
        res.json(updatedEntry);
    } else {
        res.status(404);
        throw new Error('Timetable entry not found');
    }
});

// @desc    Delete a timetable entry
// @route   DELETE /api/timetables/:id
// @access  Private/Admin
const deleteTimetableEntry = asyncHandler(async (req, res) => {
    const entry = await Timetable.findById(req.params.id);

    if (entry) {
        await entry.deleteOne();
        getIO().emit('timetable_entry_deleted', req.params.id);
        res.json({ message: 'Timetable entry removed' });
    } else {
        res.status(404);
        throw new Error('Timetable entry not found');
    }
});

export {
    createTimetableEntry,
    getTimetableEntries,
    updateTimetableEntry,
    deleteTimetableEntry,
};

